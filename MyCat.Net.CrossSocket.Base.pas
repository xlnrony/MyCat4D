...
{ }
{ ****************************************************************************** }
unit MyCat.Net.CrossSocket.Base;
...
type
  ...
  ICrossConnection = interface(ICrossData)
    ...
    /// 连接状态
    /// </summary>
    property ConnectStatus: TConnectStatus read GetConnectStatus
      write SetConnectStatus;
  end;

  TCrossConnections = TDictionary<UInt64, ICrossConnection>;

  TCrossListenEvent = procedure(Sender: TObject; AListen: ICrossListen)
    of object;
  TCrossConnectEvent = procedure(Sender: TObject; AConnection: ICrossConnection)
    of object;
  TCrossDataEvent = procedure(Sender: TObject; AConnection: ICrossConnection;
...
{$ENDREGION}
    ...
    /// 监听成功事件
    /// </summary>
    property OnListened: TCrossListenEvent read GetOnListened
      write SetOnListened;

    /// <summary>
    /// 监听结束事件
    /// </summary>
    property OnListenEnd: TCrossListenEvent read GetOnListenEnd
      write SetOnListenEnd;

    /// <summary>
    /// 连接成功事件
    /// </summary>
    property OnConnected: TCrossConnectEvent read GetOnConnected
      write SetOnConnected;

    /// <summary>
    /// 连接断开事件
    /// </summary>
    property OnDisconnected: TCrossConnectEvent read GetOnDisconnected
  ...
  protected
    ...
    function _SetConnectStatus(const AStatus: TConnectStatus)
      : TConnectStatus; inline;
    procedure SetConnectStatus(const Value: TConnectStatus);
  ...
  protected const
    RCV_BUF_SIZE = 32768;
  protected
    class threadvar FRecvBuf: array [0 .. RCV_BUF_SIZE - 1] of Byte;
  protected
    FIoThreads: Integer;
...
{$REGION '物理事件'}
    ...
    procedure TriggerReceived(AConnection: ICrossConnection; ABuf: Pointer;
      ALen: Integer); virtual;
    procedure TriggerSent(AConnection: ICrossConnection; ABuf: Pointer;
      ALen: Integer); virtual;
{$ENDREGION}
{$REGION '逻辑事件'}
    // 这几个虚方法用于在派生类中使用
    ...
    procedure LogicReceived(AConnection: ICrossConnection; ABuf: Pointer;
      ALen: Integer); virtual;
    procedure LogicSent(AConnection: ICrossConnection; ABuf: Pointer;
      ALen: Integer); virtual;
{$ENDREGION}
...
begin
  ...
  begin
    try
      if not FCrossSocket.ProcessIoEvent then
        Break;
    except
{$IFDEF DEBUG}
      on e: Exception do
        _Log('%s Io线程ID %d, 异常 %s, %s',
          [TAbstractCrossSocket(FCrossSocket).ClassName, Self.ThreadID,
...
procedure TAbstractCrossSocket.SetOnDisconnected(const Value
  : TCrossConnectEvent);
begin
...
procedure TAbstractCrossSocket.TriggerDisconnected
...
begin
  _LockListens;
  try
    if not FListens.ContainsKey(AListen.UID) then
      Exit;
...
{$REGION '本地地址信息'}
  ...
  if (TSocketAPI.GetSockName(FSocket, @LAddr.Addr, LAddr.AddrLen) = 0) then
    TSocketAPI.ExtractAddrInfo(@LAddr.Addr, LAddr.AddrLen, FLocalAddr,
      FLocalPort);
{$ENDREGION}
end;
...
constructor TAbstractCrossListen.Create(AOwner: ICrossSocket;
  AListenSocket: THandle; AFamily, ASockType, AProtocol: Integer);
begin
...
procedure TAbstractCrossConnection.SetConnectStatus
  (const Value: TConnectStatus);
begin
...
begin
  ...
  FOwner.Send(LConnection, LBuffer, ACount,
    ...
    begin
      if ASuccess then
        (FOwner as TAbstractCrossSocket).TriggerSent(AConnection,
          LBuffer, ACount);
...
AOffset, ACount: Integer; const ACallback: TProc<ICrossConnection, Boolean>);
var
  LBytes: TBytes;
begin
  // 增加引用计数
...
begin
  ...
  begin
    SendBytes(TBytesStream(AStream).Bytes, TBytesStream(AStream).Position,
      TBytesStream(AStream).Size - TBytesStream(AStream).Position, ACallback);
    Exit;
  end;
...
{$REGION '远端地址信息'}
  ...
  if (TSocketAPI.GetPeerName(FSocket, @LAddr.Addr, LAddr.AddrLen) = 0) then
    TSocketAPI.ExtractAddrInfo(@LAddr.Addr, LAddr.AddrLen, FPeerAddr,
      FPeerPort);
{$ENDREGION}
end;

function TAbstractCrossConnection._SetConnectStatus(const AStatus
