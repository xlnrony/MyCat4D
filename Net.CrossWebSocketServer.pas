...
type
  ...
  ICrossWebSocketConnection = interface(ICrossHttpConnection)
    ...
    /// 回调函数
    /// </param>
    procedure WsSend(const AData; ACount: NativeInt;
    ...
    /// 回调函数
    /// </param>
    procedure WsSend(const AData: TBytes; AOffset, ACount: NativeInt;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure WsSend(const AData: TBytes;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure WsSend(const AData: string;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;
    ...
    /// </list>
    /// </remarks>
    procedure WsSend(const AData: TFunc<PPointer, PNativeInt, Boolean>;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;
    ...
    /// </remarks>
    procedure WsSend(const AData: TStream; const AOffset, ACount: Int64;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;
    ...
    /// 必须保证发送过程中流对象的有效性, 要释放流对象可以放到回调函数中进行
    /// </remarks>
    procedure WsSend(const AData: TStream;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;
  end;
  ...
  TCrossWebSocketConnection = class(TCrossHttpConnection,
    ICrossWebSocketConnection)
  private type
...
{$REGION '内部发送方法'}
    procedure _WsSend(AOpCode: Byte; AFin: Boolean; AData: Pointer;
      ACount: NativeInt;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;

    procedure _WsSend(AOpCode: Byte; AFin: Boolean; const AData: TBytes;
      AOffset, ACount: NativeInt;
      ACallback: TProc<ICrossWebSocketConnection, Boolean> = nil); overload;
...
{$ENDREGION}
  ...
  public
    ...
    class function _MakeFrameHeader(AOpCode: Byte; AFin: Boolean;
      AMaskKey: Cardinal; ADataSize: UInt64): TBytes; static;
  ...
  private
    ...
    procedure _WebSocketHandshake(ARequest: ICrossHttpRequest;
  ...
  protected
    ...
    procedure DoOnRequest(ARequest: ICrossHttpRequest;
      AResponse: ICrossHttpResponse); override;
...
procedure TCrossWebSocketConnection.TriggerWsRequest(ARequestType
  : TWsRequestType; const ARequestData: TBytes);
begin
  TNetCrossWebSocketServer(Owner).TriggerWsRequest(Self, ARequestType,
    ARequestData);
end;
...
procedure TCrossWebSocketConnection.WsSend(const AData
...
begin
  ...
  LSender :=
    ...
    begin
      ...
      LData := nil;
      LCount := 0;
      if not Assigned(AData) or not AData(@LData, @LCount) or (LData = nil) or
        (LCount <= 0) then
      begin
        LSender := nil;
      ...
      TCrossWebSocketConnection(AConnection)._WsSend(LOpCode, False, LData,
        LCount, LSender);
...
begin
  ...
  begin
    WsSend(Pointer(IntPtr(TCustomMemoryStream(AData).Memory) + LOffset)^,
      LCount, ACallback);
    Exit;
  end;
...
procedure TCrossWebSocketConnection._AdjustOffsetCount(const ABodySize
  : NativeInt; var AOffset, ACount: NativeInt);
begin
{$REGION '修正 AOffset'}
  // 偏移为正数, 从头部开始计算偏移
  if (AOffset >= 0) then
...
begin
  ...
  SetLength(Result, LHeaderSize + ADataSize);
...
function TCrossWebSocketConnection._OpCodeToReqType(AOpCode: Byte)
  : TWsRequestType;
begin
  case AOpCode of
    WS_OP_TEXT:
    ...
    case FWsFrameState of
      wsHeader:
        begin
          ...
          else if (FWsFrameHeader.Size = FWsHeaderSize) then
          begin
            FWsFrameState := wsBody;

            // 保存 mask key
            if FWsMask then
              Move(PCardinal(UIntPtr(FWsFrameHeader.Memory) + FWsHeaderSize - 4)
                ^, FWsMaskKey, 4);
...
begin
  ...
  Move(AData^, LWsFrameHeader[Length(LWsFrameHeader) - ACount], ACount);
...
function TNetCrossWebSocketServer.OnClose(ACallback: TWsOnClose)
  : ICrossWebSocketServer;
begin
  FOnClose := ACallback;
  Result := Self;
end;

function TNetCrossWebSocketServer.OnMessage(ACallback: TWsOnMessage)
  : ICrossWebSocketServer;
begin
...
function TNetCrossWebSocketServer.OnOpen(ACallback: TWsOnOpen)
  : ICrossWebSocketServer;
begin
  FOnOpen := ACallback;
  Result := Self;
...
procedure TNetCrossWebSocketServer._OnClose(AConnection
  : ICrossWebSocketConnection);
begin
...
procedure TNetCrossWebSocketServer._OnMessage(AConnection
  : ICrossWebSocketConnection; ARequestType: TWsRequestType;
const ARequestData: TBytes);
begin
...
procedure TNetCrossWebSocketServer._OnOpen(AConnection: ICrossWebSocketConnection);
begin
...
ACallback: TProc<ICrossWebSocketConnection, Boolean>);
begin
  AResponse.Header['Upgrade'] := 'websocket';
  AResponse.Header['Connection'] := 'Upgrade';
  AResponse.Header['Sec-WebSocket-Accept'] :=
    TNetEncoding.Base64.EncodeBytesToString
    (THashSHA1.GetHashBytes(ARequest.Header['Sec-WebSocket-Key'] +
    WS_MAGIC_STR));
...
begin
  Result := TCrossWebSocketConnection.Create(AOwner, AClientSocket,
    AConnectType);
end;

procedure TNetCrossWebSocketServer.DoOnRequest(ARequest: ICrossHttpRequest;
AResponse: ICrossHttpResponse);
var
  LConnection: ICrossWebSocketConnection;
begin
  LConnection := ARequest.Connection as ICrossWebSocketConnection;
  if SameText(ARequest.Header['Connection'], 'Upgrade') and
    SameText(ARequest.Header['Upgrade'], 'websocket') then
  begin
    TCrossWebSocketConnection(LConnection).FIsWebSocket := True;
    _WebSocketHandshake(ARequest, AResponse,
      procedure(AConnection: ICrossWebSocketConnection; ASuccess: Boolean)
      begin
        if ASuccess then
          _OnOpen(AConnection);
