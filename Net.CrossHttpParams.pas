...
uses
  ...
  System.RegularExpressions, System.Diagnostics,
...
type
  ...
  public
    ...
    procedure Add(const AName, AValue: string;
      ADupAllowed: Boolean = False); overload;
    ...
    /// 是否清除现有数据
    /// </param>
    procedure Decode(const AEncodedParams: string; AClear: Boolean = True);
      virtual; abstract;
    ...
    /// 按名称访问参数
    /// </summary>
    property Params[const AName: string]: string read GetParam
  ...
  public
    ...
    /// 是否清除现有数据
    /// </param>
    procedure Decode(const AEncodedParams: string;
      AClear: Boolean = True); override;
  ...
  public
    ...
    property Cookies[const AName: string]: TResponseCookie read GetCookie
      write SetCookie;
  end;
  ...
  private
    ...
    FBoundaryIndex, FDetectHeaderIndex, FDetectEndIndex, FPartDataBegin
      : Integer;
    FPrevIndex: Integer;
  ...
  ISessions = interface
    ...
    /// Session类
    /// </summary>
    property SessionClass: TSessionClass read GetSessionClass
      write SetSessionClass;
    ...
    /// Session ID
    /// </param>
    property Sessions[const ASessionID: string]: ISession
      read GetSession; default;
  ...
  public
    ...
    function ExistsSession(const ASessionID: string): Boolean; virtual;
      abstract;
    procedure AddSession(const ASessionID: string; ASession: ISession);
      virtual; abstract;
...
uses
  System.RTLConsts, Utils.Utils, Utils.DateTime;

type
  TURLEncodingEx = class(TURLEncoding)
  protected
...
begin
  ...
  begin
    ...
    URLEncodingEx := TURLEncodingEx.Create;
    try
      LName := URLEncodingEx.Decode(LName);
    finally
      URLEncodingEx.Free;
    end;
    // 跳过多余的'='
    ...
    URLEncodingEx := TURLEncodingEx.Create;
    try
      LValue := URLEncodingEx.Decode(LValue);
    finally
      URLEncodingEx.Free;
    end;
    // 跳过多余的'&'
...
begin
  ...
  begin
    ...
    begin
      URLEncodingEx := TURLEncodingEx.Create;
      try
        Result := Result + URLEncodingEx.Encode(FParams[I].Name)
      finally
        URLEncodingEx.Free;
      end;
    end
    ...
    begin
      URLEncodingEx := TURLEncodingEx.Create;
      try
        Result := Result + '=' + URLEncodingEx.Encode(FParams[I].Value)
      finally
        URLEncodingEx.Free;
      end;
    end
...
begin
  ...
  begin
    ...
    URLEncodingEx := TURLEncodingEx.Create;
    try
      Result := Result + FParams[I].Name + '=' + URLEncodingEx.Encode
        (FParams[I].Value);
    finally
      URLEncodingEx.Free;
    end;
...
begin
  URLEncodingEx := TURLEncodingEx.Create;
  try
    Result := Name + '=' + URLEncodingEx.Encode(Value);
  finally
    URLEncodingEx.Free;
  end;
  ...
  begin
    ...
    Result := Result + '; Expires=' + TCrossHttpUtils.RFC1123_DateToStr
...
function THttpMultiPartFormData.Decode(const ABuf: Pointer;
ALen: Integer): Integer;
  ...
  procedure __InitFormFieldByHeader(AFormField: TFormField;
  ...
  begin
    ...
    try
      ...
      if (LContentDisposition = '') then
        Exit;

      LMatch := TRegEx.Match(LContentDisposition, '\bname="(.*?)"(?=;|$)',
        [TRegExOption.roIgnoreCase]);
      if LMatch.Success then
        AFormField.FName := LMatch.Groups[1].Value;

      LMatch := TRegEx.Match(LContentDisposition, '\bfilename="(.*?)"(?=;|$)',
        [TRegExOption.roIgnoreCase]);
      if LMatch.Success then
      begin
        ...
        AFormField.FValue := TFile.Open(AFormField.FFilePath,
          TFileMode.fmOpenOrCreate, TFileAccess.faReadWrite, TFileShare.fsRead);
      end
      else
        AFormField.FValue := TBytesStream.Create(nil);

      AFormField.FContentType := LFieldHeader['Content-Type'];
      AFormField.FContentTransferEncoding :=
        LFieldHeader['Content-Transfer-Encoding'];
    finally
    ...
    case FDecodeState of
      ...
      dsPartHeader:
        begin
          ...
          begin
            // 块头部通常采用UTF8编码
            LPartHeader := TEncoding.UTF8.GetString(FCurrentPartHeader.Bytes, 0,
              FCurrentPartHeader.Size - 4 { #13#10#13#10 } );
            FCurrentPartHeader.Clear;
      ...
      dsPartData:
        begin
          ...
          begin
            // 将内存块数据存入Field中
            if (FPartDataBegin >= 0) then
              FCurrentPartField.FValue.Write(p[FPartDataBegin],
                I - FPartDataBegin - FBoundaryIndex + 1);
...
AMaxAge: Integer; const APath, ADomain: string; AHttpOnly, ASecure: Boolean);
begin
  SetCookie(AName, TResponseCookie.Create(AName, AValue, AMaxAge, APath,
    ADomain, AHttpOnly, ASecure));
end;
...
begin
  TThread.CreateAnonymousThread(
    procedure procedure _ClearExpiredSessions;
    var
...
function TURLEncodingEx.DoDecode(const Input: string): string;
  ...
  try
    ...
    on E: EConvertError do
      raise EConvertError.CreateFmt(sInvalidURLEncodedChar,
        [Char('%') + Cp^ + Sp^, Cp - PChar(Input)])
  end;
  SetLength(Bytes, I);
  I := TEncoding.UTF8.GetCharCount(Bytes, 0, Length(Bytes));
  if (Length(Bytes) > 0) and (I = 0) then
  begin
