...
uses
  ...
  DSTL.Types, DSTL.STL.Iterator, DSTL.STL.Sequence, DSTL.Exception,
  DSTL.STL.Alloc,
...
type
  ...
  protected
    ...
    function insert_aux(Iterator: TIterator<T>; const obj: T)
      : TIterator<T>; overload;
    procedure insert_aux(position: TIterator<T>; first, last: TIterator<T>;
      n: integer); overload;
    procedure fill_insert(position: TIterator<T>; n: integer; const obj: T);
    procedure insert_aux(position: TIterator<T>; n: integer;
      const obj: T); overload;
  public
    ...
    function insert(Iterator: TIterator<T>; const obj: T)
      : TIterator<T>; overload;
    procedure insert(position: TIterator<T>; n: integer; const obj: T);
      overload;
    procedure insert(position: TIterator<T>;
      first, last: TIterator<T>); overload;
...
var
  It: TIterator<T>;
begin
  if (idx >= size) or (idx < 0) then
    dstl_raise_exception(E_OUT_OF_BOUND);
  It := fstart;
  while idx > 0 do
  begin
    iadvance(It);
...
begin
  if value > MAX_BUFFER_SIZE then
    raise EDSTLBufferSizeTooLargeException.Create
      ('Buffer size should be less than ' + inttostr(MAX_BUFFER_SIZE));
  Self.buf_size := value;
end;
...
function TDeque<T>.remove(const obj: T): boolean;
var
  It: TIterator<T>;
...
begin
  ...
  It := fstart;
  while (It.bnode <> nil) and (not empty) do
  begin
    if comparer.Equals(It.bnode.buf[It.cur], obj) then
    begin
      Result := True;
      erase(It);
    end;
...
begin
  ...
  if fstart.bnode <> ffinish.bnode then
    allocator.deallocate(ffinish.bnode.buf, buf_size * sizeOf(T));
  ffinish := fstart;
end;
...
begin
  ...
  begin
    ...
    Result := insert_aux(Iterator, obj);
end;

function TDeque<T>.insert_aux(Iterator: TIterator<T>; const obj: T)
  : TIterator<T>;
var
  index: integer;
...
procedure TDeque<T>.fill_insert(position: TIterator<T>; n: integer;
  const obj: T);
var
...
procedure TDeque<T>.insert_aux(position: TIterator<T>; n: integer;
  const obj: T);
var
...
procedure TDeque<T>.insert_aux(position: TIterator<T>;
  first, last: TIterator<T>; n: integer);
var
