...
type
  ...
  private
    FStatusCode: Integer;
  public
    constructor Create(const AMessage: string; AStatusCode: Integer = 500);
      reintroduce; virtual;
    constructor CreateFmt(const AMessage: string; const AArgs: array of const;
      AStatusCode: Integer = 500); reintroduce; virtual;
  ...
  ICrossHttpRequest = interface
    ...
    function GetBodyStream: TBytesStream;
    ...
    /// KeepAliv标志
    /// </summary>
    property BodyStream: TBytesStream read GetBodyStream;
    ...
    /// 简单认证用户名
    /// </summary>
    property AuthName: string read GetAuthName;

    /// <summary>
    /// 简单认证密码
    /// </summary>
    property AuthPass: string read GetAuthPass;
  ...
  ICrossHttpResponse = interface
    ...
    /// </remarks>
    procedure SendZCompress(const AChunkSource: TFunc<PPointer, PNativeInt,
      Boolean>; ACompressType: TCompressType;
    ...
    /// 回调函数
    /// </param>
    procedure SendZCompress(const ABody; ACount: NativeInt;
      ACompressType: TCompressType;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure SendZCompress(const ABody: TBytes; AOffset, ACount: NativeInt;
      ACompressType: TCompressType;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure SendZCompress(const ABody: TBytes; ACompressType: TCompressType;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// </remarks>
    procedure SendZCompress(const ABody: TStream; const AOffset, ACount: Int64;
      ACompressType: TCompressType;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 必须保证发送过程中流对象的有效性, 要释放流对象可以放到回调函数中进行
    /// </remarks>
    procedure SendZCompress(const ABody: TStream; ACompressType: TCompressType;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure SendZCompress(const ABody: string; ACompressType: TCompressType;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// </remarks>
    procedure SendNoCompress(const AChunkSource: TFunc<PPointer, PNativeInt,
      Boolean>; ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure SendNoCompress(const ABody; ACount: NativeInt;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure SendNoCompress(const ABody: TBytes; AOffset, ACount: NativeInt;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure SendNoCompress(const ABody: TBytes;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 必须保证发送过程中流对象的有效性, 要释放流对象可以放到回调函数中进行
    /// </remarks>
    procedure SendNoCompress(const ABody: TStream; const AOffset, ACount: Int64;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 必须保证发送过程中流对象的有效性, 要释放流对象可以放到回调函数中进行
    /// </remarks>
    procedure SendNoCompress(const ABody: TStream;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数
    /// </param>
    procedure SendNoCompress(const ABody: string;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数 <br />
    /// </param>
    procedure Send(const ABody; ACount: NativeInt;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数 <br />
    /// </param>
    procedure Send(const ABody: TBytes; AOffset, ACount: NativeInt;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数 <br />
    /// </param>
    procedure Send(const ABody: TBytes;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 必须保证发送过程中流对象的有效性, 要释放流对象可以放到回调函数中进行
    /// </remarks>
    procedure Send(const ABody: TStream; const AOffset, ACount: Int64;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 必须保证发送过程中流对象的有效性, 要释放流对象可以放到回调函数中进行
    /// </remarks>
    procedure Send(const ABody: TStream;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    ...
    /// 回调函数 <br />
    /// </param>
    procedure Send(const ABody: string;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
  ...
  ICrossHttpRouter = interface
    ...
    procedure Execute(ARequest: ICrossHttpRequest;
  ...
  TCrossHttpRouterProc = reference to procedure(ARequest: ICrossHttpRequest;
    AResponse: ICrossHttpResponse);
  TCrossHttpRouterMethod = procedure(ARequest: ICrossHttpRequest;
    AResponse: ICrossHttpResponse) of object;
  TCrossHttpRouterProc2 = reference to procedure(ARequest: ICrossHttpRequest;
    AResponse: ICrossHttpResponse; var AHandled: Boolean);
  TCrossHttpRouterMethod2 = procedure(ARequest: ICrossHttpRequest;
    AResponse: ICrossHttpResponse; var AHandled: Boolean) of object;

  TCrossHttpConnEvent = procedure(Sender: TObject;
    AConnection: ICrossHttpConnection) of object;
  TCrossHttpPostEvent = procedure(Sender: TObject; ARequest: ICrossHttpRequest)
    of object;
  TCrossHttpRequestEvent = procedure(Sender: TObject;
    ARequest: ICrossHttpRequest; AResponse: ICrossHttpResponse;
    var AHandled: Boolean) of object;
  TCrossHttpRequestExceptionEvent = procedure(Sender: TObject;
    ARequest: ICrossHttpRequest; AResponse: ICrossHttpResponse;
    AException: Exception) of object;
  TCrossHttpDataEvent = procedure(Sender: TObject; ARequest: ICrossHttpRequest;
    ABuf: Pointer; ALen: Integer) of object;
  ...
  ICrossHttpServer = interface({$IFDEF __CROSS_SSL__}ICrossSslServer{$ELSE}ICrossServer{$ENDIF})
    ...
    function GetOnPostDataBegin: TCrossHttpPostEvent;
    function GetOnPostData: TCrossHttpDataEvent;
    function GetOnPostDataEnd: TCrossHttpPostEvent;
    ...
    procedure SetOnRequestException(const Value
      : TCrossHttpRequestExceptionEvent);
    procedure SetOnPostDataBegin(const Value: TCrossHttpPostEvent);
    procedure SetOnPostData(const Value: TCrossHttpDataEvent);
    procedure SetOnPostDataEnd(const Value: TCrossHttpPostEvent);
    ...
    /// </list>
    /// </remarks>
    function Use(const APath: string; AMiddlewareProc: TCrossHttpRouterProc)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Use(const APath: string; AMiddlewareProc2: TCrossHttpRouterProc2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Use(const APath: string; AMiddlewareMethod: TCrossHttpRouterMethod)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Use(AMiddlewareProc: TCrossHttpRouterProc)
      : ICrossHttpServer; overload;
    function Use(AMiddlewareProc2: TCrossHttpRouterProc2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Use(AMiddlewareMethod: TCrossHttpRouterMethod)
      : ICrossHttpServer; overload;
    function Use(AMiddlewareMethod2: TCrossHttpRouterMethod2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Route(const AMethod, APath: string;
      ARouterProc: TCrossHttpRouterProc): ICrossHttpServer; overload;
    function Route(const AMethod, APath: string;
      ARouterProc2: TCrossHttpRouterProc2): ICrossHttpServer; overload;
    ...
    /// </remarks>
    function Route(const AMethod, APath: string;
      ARouterMethod: TCrossHttpRouterMethod): ICrossHttpServer; overload;
    function Route(const AMethod, APath: string;
      ARouterMethod2: TCrossHttpRouterMethod2): ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Get(const APath: string; ARouterProc: TCrossHttpRouterProc)
      : ICrossHttpServer; overload;
    function Get(const APath: string; ARouterProc2: TCrossHttpRouterProc2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Get(const APath: string; ARouterMethod: TCrossHttpRouterMethod)
      : ICrossHttpServer; overload;
    function Get(const APath: string; ARouterMethod2: TCrossHttpRouterMethod2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Put(const APath: string; ARouterProc: TCrossHttpRouterProc)
      : ICrossHttpServer; overload;
    function Put(const APath: string; ARouterProc2: TCrossHttpRouterProc2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Put(const APath: string; ARouterMethod: TCrossHttpRouterMethod)
      : ICrossHttpServer; overload;
    function Put(const APath: string; ARouterMethod2: TCrossHttpRouterMethod2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Post(const APath: string; ARouterProc: TCrossHttpRouterProc)
      : ICrossHttpServer; overload;
    function Post(const APath: string; ARouterProc2: TCrossHttpRouterProc2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Post(const APath: string; ARouterMethod: TCrossHttpRouterMethod)
      : ICrossHttpServer; overload;
    function Post(const APath: string; ARouterMethod2: TCrossHttpRouterMethod2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Delete(const APath: string; ARouterProc: TCrossHttpRouterProc)
      : ICrossHttpServer; overload;
    function Delete(const APath: string; ARouterProc2: TCrossHttpRouterProc2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function Delete(const APath: string; ARouterMethod: TCrossHttpRouterMethod)
      : ICrossHttpServer; overload;
    function Delete(const APath: string;
      ARouterMethod2: TCrossHttpRouterMethod2): ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function All(const APath: string; ARouterProc: TCrossHttpRouterProc)
      : ICrossHttpServer; overload;
    function All(const APath: string; ARouterProc2: TCrossHttpRouterProc2)
      : ICrossHttpServer; overload;
    ...
    /// </list>
    /// </remarks>
    function All(const APath: string; ARouterMethod: TCrossHttpRouterMethod)
      : ICrossHttpServer; overload;
    function All(const APath: string; ARouterMethod2: TCrossHttpRouterMethod2)
      : ICrossHttpServer; overload;
    ...
    /// 对象释放时自动删除上传的文件
    /// </summary>
    property AutoDeleteFiles: Boolean read GetAutoDeleteFiles
      write SetAutoDeleteFiles;
    ...
    /// </list>
    /// </summary>
    property MaxPostDataSize: Int64 read GetMaxPostDataSize
      write SetMaxPostDataSize;
    ...
    /// </list>
    /// </remarks>
    property MinCompressSize: Int64 read GetMinCompressSize
      write SetMinCompressSize;
    ...
    /// </remarks>
    property SessionIDCookieName: string read GetSessionIDCookieName
      write SetSessionIDCookieName;

    property OnRequest: TCrossHttpRequestEvent read GetOnRequest
      write SetOnRequest;
    property OnRequestException: TCrossHttpRequestExceptionEvent
      read GetOnRequestException write SetOnRequestException;
    property OnPostDataBegin: TCrossHttpPostEvent read GetOnPostDataBegin
      write SetOnPostDataBegin;
    property OnPostData: TCrossHttpDataEvent read GetOnPostData
      write SetOnPostData;
    property OnPostDataEnd: TCrossHttpPostEvent read GetOnPostDataEnd
      write SetOnPostDataEnd;
  end;
  ...
  private
    ...
    function NewRequest: TCrossHttpRequest;
    function NewResponse(ARequest: ICrossHttpRequest): TCrossHttpResponse;
  ...
  private
    ...
    function GetContentType: string;
  private type
    TCrossHttpParseState = (psHeader, psPostData, psChunkSize, psChunkData,
      psChunkEnd, psDone);
  private
...
{$REGION '内部: 基础发送方法'}
    procedure _Send(ASource: TFunc<PPointer, PNativeInt, Boolean>;
      ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
    procedure _Send(AHeaderSource, ABodySource: TFunc<PPointer, PNativeInt,
      Boolean>; ACallback: TProc<ICrossConnection, Boolean> = nil); overload;
{$ENDREGION}
    function _CheckCompress(const ABodySize: Int64;
...
{$ENDREGION}
  ...
  private
    ...
    function MakePathPattern(const APath: string;
  ...
  private const
    ...
    HTTP_METHODS: array [0 .. HTTP_METHOD_COUNT - 1] of string = ('GET', 'POST',
      'PUT', 'DELETE', 'HEAD', 'OPTIONS', 'TRACE', 'CONNECT', 'PATCH', 'COPY',
  ...
  private
    ...
    FOnPostDataBegin: TCrossHttpPostEvent;
    FOnPostData: TCrossHttpDataEvent;
    FOnPostDataEnd: TCrossHttpPostEvent;
    FCompressible: Boolean;

    function IsValidHttpRequest(ABuf: Pointer; ALen: Integer): Boolean;
    procedure ParseRecvData(AConnection: ICrossConnection; ABuf: Pointer;
      ALen: Integer);
  ...
  protected
    ...
    procedure LogicReceived(AConnection: ICrossConnection; ABuf: Pointer;
      ALen: Integer); override;

    procedure LogicDisconnected(AConnection: ICrossConnection); override;
  protected
    procedure TriggerPostDataBegin(ARequest: TCrossHttpRequest); virtual;
    procedure TriggerPostData(ARequest: TCrossHttpRequest; const ABuf: Pointer;
      ALen: Integer); virtual;
    procedure TriggerPostDataEnd(ARequest: TCrossHttpRequest); virtual;

    // 处理请求
    procedure DoOnRequest(ARequest: ICrossHttpRequest;
      AResponse: ICrossHttpResponse); virtual;
  public
...
function TCrossHttpConnection.NewRequest: TCrossHttpRequest;
begin
  Result := TCrossHttpRequest.Create(Self);
  FRequest := Result;
end;

function TCrossHttpConnection.NewResponse(ARequest: ICrossHttpRequest)
  : TCrossHttpResponse;
begin
  Result := TCrossHttpResponse.Create(Self, ARequest);
  FResponse := Result;
end;
...
function TCrossHttpRouter.IsMatch(ARequest: ICrossHttpRequest): Boolean;
  ...
  begin
    ...
    if (FPath = '*') or ((Length(FPathParamKeys) = 0) and
      SameText(ARequest.Path, FPath)) then
      Exit(True);
...
procedure TCrossHttpRouter.Execute(ARequest: ICrossHttpRequest;
AResponse: ICrossHttpResponse; var AHandled: Boolean);
begin
  ...
  else if Assigned(FRouterMethod) then
  begin
    FRouterMethod(ARequest, AResponse);
  end
  else if Assigned(FRouterProc2) then
  begin
    FRouterProc2(ARequest, AResponse, AHandled);
  end
  else if Assigned(FRouterMethod2) then
  begin
...
begin
  ...
  FStoragePath := TPath.Combine(TUtils.AppPath, 'temp') +
...
procedure TCrossHttpServer.DoOnRequest(ARequest: ICrossHttpRequest;
AResponse: ICrossHttpResponse);
var
  LSessionID: string;
  LHandled: Boolean;
...
{$REGION 'Session'}
    ...
    begin
      LSessionID := ARequest.Cookies[FSessionIDCookieName];
      (ARequest as TCrossHttpRequest).FSession := FSessions.Sessions
        [LSessionID];
      if (ARequest.Session <> nil) and (ARequest.Session.SessionID <> LSessionID)
      then
      begin
        LSessionID := ARequest.Session.SessionID;
        AResponse.Cookies.AddOrSet(FSessionIDCookieName, LSessionID, 0);
      end;
    end;
{$ENDREGION}
{$REGION '中间件'}
    ...
    begin
      // 执行匹配的中间件
      if LMiddleware.IsMatch(ARequest) then
      begin
        // 中间件通常用于请求的预处理
        // 所以默认将 LHandled 置为 False, 以保证后续路由能被执行
        // 除非用户在中间件中明确指定了 LHandled := True, 表明该请求无需后续路由响应了
        LHandled := False;
        LMiddleware.Execute(ARequest, AResponse, LHandled);

        // 如果已经发送了数据, 则后续的事件和路由响应都不需要执行了
        if LHandled or AResponse.Sent then
          Exit;
      end;
    end;
{$ENDREGION}
{$REGION '响应请求事件'}
    ...
    begin
      LHandled := False;
      FOnRequest(Self, ARequest, AResponse, LHandled);
...
{$REGION '路由'}
    ...
    begin
      // 执行匹配的路由
      if LRouter.IsMatch(ARequest) then
      begin
        // 路由用于响应请求
        ...
        LHandled := True;
        LRouter.Execute(ARequest, AResponse, LHandled);
...
{$ENDREGION}
    ...
    if not(LHandled or AResponse.Sent) then
      AResponse.SendStatus(404);
  except
    on e: Exception do
    begin
      if Assigned(FOnRequestException) then
        FOnRequestException(Self, ARequest, AResponse, e)
      else if (e is ECrossHttpException) then
        AResponse.SendStatus(ECrossHttpException(e).StatusCode,
          ECrossHttpException(e).Message)
      else
        AResponse.SendStatus(500, e.Message);
    end;
  end;
end;
...
function TCrossHttpServer.GetOnPostDataBegin: TCrossHttpPostEvent;
begin
  Result := FOnPostDataBegin;
end;

function TCrossHttpServer.GetOnPostDataEnd: TCrossHttpPostEvent;
begin
...
function TCrossHttpServer.GetOnRequestException
  : TCrossHttpRequestExceptionEvent;
begin
...
procedure TCrossHttpServer.SetOnPostDataBegin(const Value: TCrossHttpPostEvent);
begin
  FOnPostDataBegin := Value;
end;

procedure TCrossHttpServer.SetOnPostDataEnd(const Value: TCrossHttpPostEvent);
begin
...
procedure TCrossHttpServer.SetOnRequestException(const Value
  : TCrossHttpRequestExceptionEvent);
begin
...
function TCrossHttpServer.Static(const APath, ALocalStaticDir: string)
  : ICrossHttpServer;
var
  LReqPath: string;
begin
...
begin
  ...
  begin
    ...
    if (ALen >= Length(LBytes)) and CompareMem(ABuf, Pointer(LBytes),
      Length(LBytes)) then
      Exit(True);
  end;
  Result := False;
...
var
  LHttpConnection: TCrossHttpConnection;
  ...
  begin
    LResponse.SendStatus(AStatusCode, AErrMsg);
  end;

begin
  LHttpConnection := AConnection as TCrossHttpConnection;

  // 在这里解析客户端浏览器发送过来的请求数据
  if Assigned(LHttpConnection.Request) then
  begin
    LRequest := LHttpConnection.Request as TCrossHttpRequest;
  end
  else
  begin
    LRequest := LHttpConnection.NewRequest;
  end;
  if Assigned(LHttpConnection.Response) then
  begin
    LResponse := LHttpConnection.Response as TCrossHttpResponse;
  end
  else
  begin
    LResponse := LHttpConnection.NewResponse(LRequest);
  end;
    ...
    case LRequest.FParseState of
      psHeader:
        begin
          ...
          // Header尺寸超标
          if (FMaxHeaderSize > 0) and
            (LRequest.FRawRequest.Size + 1 > FMaxHeaderSize) then
          begin
          ...
          if (LRequest.FRawRequest.Size = 8) and
            not IsValidHttpRequest(LRequest.FRawRequest.Memory,
            LRequest.FRawRequest.Size) then
          begin
          ...
          begin
            ...
            // Post数据尺寸超标, 直接断开连接
            if (FMaxPostDataSize > 0) and
              (LRequest.FContentLength > FMaxPostDataSize) then
            begin
      ...
      psPostData:
        begin
          LChunkSize :=
            Min((LRequest.ContentLength - LRequest.FPostDataSize), ALen);
          // Post数据尺寸超标, 直接断开连接
          if (FMaxPostDataSize > 0) and
            (LRequest.FPostDataSize + LChunkSize > FMaxPostDataSize) then
          begin
            _Error(400, 'Post data too large.');
            Exit;
          end;
          TriggerPostData(LRequest, pch, LChunkSize);
      ...
      psChunkSize:
        begin
          ...
          begin
            SetString(LLineStr,
              MarshaledAString(LRequest.FChunkSizeStream.Memory),
              LRequest.FChunkSizeStream.Size);
  ...
  begin
    DoOnRequest(LRequest, LResponse);
    LHttpConnection.FRequest := nil;
    LHttpConnection.FResponse := nil;
  end;
...
begin
  LMiddleware := TCrossHttpRouter.Create(AMethod, APath, AMiddlewareProc,
    AMiddlewareMethod, AMiddlewareProc2, AMiddlewareMethod2);
  FMiddlewaresLock.BeginWrite;
  try
...
begin
  LRouter := TCrossHttpRouter.Create(AMethod, APath, ARouterProc, ARouterMethod,
...
function TCrossHttpServer.RemoveRouter(const AMethod, APath: string)
  : ICrossHttpServer;
var
  I: Integer;
begin
  ...
  try
    for I := FRouters.Count - 1 downto 0 do
      if SameText(TCrossHttpRouter(FRouters[I]).FMethod, AMethod) and
        SameText(TCrossHttpRouter(FRouters[I]).FPath, APath) then
        FRouters.Delete(I);
  finally
    FRoutersLock.EndWrite;
  end;
...
procedure TCrossHttpServer.TriggerPostDataBegin(ARequest: TCrossHttpRequest);
var
  LMultiPart: THttpMultiPartFormData;
  LStream: TBytesStream;
begin
  LStream := TBytesStream.Create(nil);
  FreeAndNil(ARequest.FBodyStream);
  ARequest.FBodyStream := LStream;
  ...
  case ARequest.BodyType of
    btMultiPart:
      begin
        ...
        LMultiPart.InitWithBoundary(ARequest.RequestBoundary);
        FreeAndNil(ARequest.FBody);
        ARequest.FBody := LMultiPart;
      end;
  ...
  if Assigned(FOnPostDataBegin) then
    FOnPostDataBegin(Self, ARequest);
end;

procedure TCrossHttpServer.TriggerPostData(ARequest: TCrossHttpRequest;
const ABuf: Pointer; ALen: Integer);
begin
  case ARequest.GetBodyType of
    btMultiPart:
      begin
        (ARequest.Body as THttpMultiPartFormData).Decode(ABuf, ALen);
      end;
    // btUrlEncoded:
    // begin
    // (ARequest.Body as TStream).Write(ABuf^, ALen);
    // end;
    // btBinary:
    // begin
    // (ARequest.Body as TStream).Write(ABuf^, ALen);
  ...
  ARequest.BodyStream.Write(ABuf^, ALen);

  if Assigned(FOnPostData) then
    FOnPostData(Self, ARequest, ABuf, ALen);
end;

procedure TCrossHttpServer.TriggerPostDataEnd(ARequest: TCrossHttpRequest);
var
...
begin
  case ARequest.GetBodyType of
    btUrlEncoded:
      begin
        SetString(LUrlEncodedStr, MarshaledAString(ARequest.BodyStream.Memory),
          ARequest.BodyStream.Size);
        LUrlEncodedBody := THttpUrlParams.Create;
        LUrlEncodedBody.Decode(LUrlEncodedStr);
        FreeAndNil(ARequest.FBody);
        ARequest.FBody := LUrlEncodedBody;
      end;

    // btBinary:
    // begin
    // (ARequest.Body as TStream).Position := 0;
    // end;
  end;

  ARequest.BodyStream.Position := 0;

  if Assigned(FOnPostDataEnd) then
    FOnPostDataEnd(Self, ARequest);
end;
...
procedure TCrossHttpServer.LogicDisconnected(AConnection: ICrossConnection);
var
  LHttpConnection: TCrossHttpConnection;
begin
  inherited;
...
begin
  Result := RegisterMiddleware(AMethod, APath, nil, AMiddlewareMethod,
    nil, nil);
end;
...
function TCrossHttpServer.Use(AMiddlewareMethod: TCrossHttpRouterMethod)
  : ICrossHttpServer;
begin
...
function TCrossHttpServer.Use(AMiddlewareMethod2: TCrossHttpRouterMethod2)
  : ICrossHttpServer;
begin
  Result := Use('*', '*', AMiddlewareMethod2);
end;

function TCrossHttpServer.Use(AMiddlewareProc2: TCrossHttpRouterProc2)
  : ICrossHttpServer;
begin
...
begin
  Result := RegisterMiddleware(AMethod, APath, nil, nil, nil,
    AMiddlewareMethod2);
end;
...
function TCrossHttpServer.Use(AMiddlewareProc: TCrossHttpRouterProc)
  : ICrossHttpServer;
begin
...
function TCrossHttpRequest.GetAuthName: string;
begin
  Result := FAuthName;
end;
...
function TCrossHttpRequest.GetAuthPass: string;
begin
  Result := FAuthPass;
end;
...
function TCrossHttpRequest.GetBodyStream: TBytesStream;
begin
  Result := FBodyStream;
end;
...
begin
  SetString(FRawRequestText, MarshaledAString(FRawRequest.Memory),
    FRawRequest.Size);
  ...
  if (FAuthorization <> '') and FAuthorization.StartsWith('Basic') then
  begin
    LAuthStr := FAuthorization.Substring(6);
    if (LAuthStr <> '') then
    begin
  ...
  FIfModifiedSince := TCrossHttpUtils.RFC1123_StrToDate
    (FHeader['If-Modified-Since']);
...
procedure TCrossHttpResponse.Redirect(const AUrl: string;
ACallback: TProc<ICrossConnection, Boolean>);
begin
  SetLocation(AUrl);
...
AOffset, ACount: NativeInt; ACallback: TProc<ICrossConnection, Boolean>);
var
  LBody: TBytes;
...
const AOffset, ACount: Int64; ACallback: TProc<ICrossConnection, Boolean>);
var
  LCompressType: TCompressType;
begin
...
procedure TCrossHttpResponse.SendNoCompress(const AChunkSource
  : TFunc<PPointer, PNativeInt, Boolean>;
ACallback: TProc<ICrossConnection, Boolean>);
{
  HTTP头\r\n\r\n
  块尺寸\r\n
      ...
      case LChunkState of
        csHead:
          begin
            LChunkData := nil;
            LChunkSize := 0;
            if not Assigned(AChunkSource) or not AChunkSource(@LChunkData,
              @LChunkSize) or (LChunkData = nil) or (LChunkSize <= 0) then
            begin
              LChunkState := csDone;
            ...
            LChunkHeader := TEncoding.ANSI.GetBytes(IntToHex(LChunkSize, 0))
              + [13, 10];
...
begin
  ...
  try
    ...
    if (LRequest.IfModifiedSince > 0) and
      (LRequest.IfModifiedSince >= (LLastModified - (1 / SecsPerDay))) then
    begin
    ...
    LETag := '"' + TUtils.BytesToHex
      (THashMD5.GetHashBytes(AFileName + LLastModifiedStr)) + '"';
    ...
    LStream := TFile.Open(AFileName, TFileMode.fmOpen, TFileAccess.faRead,
  ...
  if (LRangeStr <> '') and ((LRequest.IfRange = '') or
    (LRequest.IfRange = LETag)) then
  begin
    ...
    else if (Length(LRangeStrArr) >= 1) then
    begin
      LRangeBegin := StrToInt64Def(LRangeStrArr[0], 0);
...
begin
  ...
  if LServer.Compressible and (ABodySize > 0) and
    ((LServer.MinCompressSize <= 0) or (ABodySize >= LServer.MinCompressSize))
    and ((Pos('text/', LContType) > 0) or (Pos('application/json', LContType) >
    0) or (Pos('javascript', LContType) > 0) or (Pos('xml', LContType) > 0))
  then
  begin
    ...
    else if (Pos('deflate', LRequestAcceptEncoding) > 0) then
    begin
...
procedure TCrossHttpResponse._Send(ASource: TFunc<PPointer, PNativeInt,
  Boolean>; ACallback: TProc<ICrossConnection, Boolean>);
var
...
begin
  ...
  LSender := procedure(AConnection: ICrossConnection; ASuccess: Boolean)
    var
      LData: Pointer;
      LCount: NativeInt;
    begin
      ...
      LData := nil;
      LCount := 0;
      if not Assigned(ASource) or not ASource(@LData, @LCount) or (LData = nil)
        or (LCount <= 0) then
      begin
        if Assigned(ACallback) then
          ACallback(AConnection, True);

        if not LKeepAlive or (LStatusCode >= 400 { 如果发送的是出错状态码, 则发送完成之后断开连接 } )
        then
...
begin
  ...
  begin
    SendNoCompress(Pointer(IntPtr(TCustomMemoryStream(ABody).Memory) + LOffset)
      ^, LCount, ACallback);
    Exit;
  end;
...
ACallback: TProc<ICrossConnection, Boolean>);
begin
  SendStatus(AStatusCode, TCrossHttpUtils.GetHttpStatusText(AStatusCode),
    ACallback);
end;

procedure TCrossHttpResponse.SendZCompress(const AChunkSource
  : TFunc<PPointer, PNativeInt, Boolean>; ACompressType: TCompressType;
ACallback: TProc<ICrossConnection, Boolean>);
{
...
const
  WINDOW_BITS: array [TCompressType] of Integer = (15 + 16 { gzip } ,
    15 { deflate } );
...
begin
  ...
  if (deflateInit2(LZStream, Z_DEFAULT_COMPRESSION, Z_DEFLATED,
    WINDOW_BITS[ACompressType], 8, Z_DEFAULT_STRATEGY) <> Z_OK) then
  begin
    if Assigned(ACallback) then
...
begin
  ...
  // CHUNK
    ...
    end, ACompressType, ACallback);
end;

procedure TCrossHttpResponse.SendZCompress(const ABody: TBytes;
AOffset, ACount: NativeInt; ACompressType: TCompressType;
ACallback: TProc<ICrossConnection, Boolean>);
var
  LBody: TBytes;
  LOffset, LCount: NativeInt;
...
const AOffset, ACount: Int64; ACompressType: TCompressType;
ACallback: TProc<ICrossConnection, Boolean>);
var
  LOffset, LCount: Int64;
  LBody: TStream;
...
begin
  ...
  if (ABody is TCustomMemoryStream) then
  begin
    SendZCompress(Pointer(IntPtr(TCustomMemoryStream(ABody).Memory) + LOffset)^,
      LCount, ACompressType, ACallback);
    Exit;
  end;
